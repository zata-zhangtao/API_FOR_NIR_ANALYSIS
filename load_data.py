...
def get_Conv_data(dir_path = "..\PD"):
    ...
    Returns:
    -----
        PD_mW_band1 (ndarray): 样本PD值，当前端口1，波段1
        PD_mW_band2 (ndarray): 样本PD值，当前端口1，波段2
        PD_mW_band3 (ndarray): 样本PD值，当前端口1，波段3
        PD_mW_band4 (ndarray): 样本PD值，当前端口1，波段4
        PD_source_mW_band1 (ndarray): 光源PD值，当前端口3，波段1
        PD_source_mW_band2 (ndarray): 光源PD值，当前端口3，波段2
        PD_source_mW_band3 (ndarray): 光源PD值，当前端口4，波段3
        PD_source_mW_band4 (ndarray): 光源PD值，当前端口4，波段4
    ...
    if len(files) == 0:
        ...
        PD_file_path = join(dir_path, files[0])

    PD_data = pd.read_csv(PD_file_path)
    try:
        PD_dBmA = PD_data.iloc[:, 1:].values
        Ch_num = PD_data.iloc[:, 0].values
        ...
        # 样本PD值
        PD_mW_band1 = PD_mW[:, 3][:200]  # Current port 1 band 1
        PD_mW_band2 = PD_mW[:, 3][200:400]   # Current port 1 band 2
        PD_mW_band3 = PD_mW[:, 3][400:600]   # Current port 1 band 3
        PD_mW_band4 = PD_mW[:, 3][600:800]   # Current port 1 band 4

        # 光源PD
        PD_source_mW_band1 = PD_mW[:, 0][:200]  # Current port 3 band 1
        PD_source_mW_band2 = PD_mW[:, 0][200:400]  # Current port 3 band 2
        PD_source_mW_band3 = PD_mW[:, 0][400:600]  # Current port 4 band 3
        PD_source_mW_band4 = PD_mW[:, 0][600:800]  # Current port 4 band 4


        return PD_mW_band1, PD_mW_band2, PD_mW_band3, PD_mW_band4, PD_source_mW_band1, PD_source_mW_band2, PD_source_mW_band3, PD_source_mW_band4
