...
def run_regression_optuna_v3(data_name,X = None,y=None ,data_splited = None,  model='PLS',split = 'SPXY',test_size = 0.3, n_trials=200,object = None,cv = None,save_dir = None,each_class_mae=False):
    ...
    if data_splited is not None:
        ...
        if split == 'SPXY':
            X_train, X_test, y_train, y_test = custom_train_test_split(X, y, test_size, 'SPXY')
            _, X_val, _, y_val = custom_train_test_split(X_train, y_train, 0.3, 'SPXY')
        elif split == "Random":
            X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=42)
